...
* `['config']` - These define the [indexes.conf] stanzas (in fairly raw form). However there are a few special keys:
    ...
    * `_noGenerateTstatsHomePath` - Do not generate tstatsHomePath for this index when set to true. Used to disable datamodel acceleration in case of the smartstore enabled indexes.This is depricated, instead of this please set the remotePath in smartstore enabled clusters.
