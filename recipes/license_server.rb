...
raise 'License Server installation not currently supported on windows' if platform_family?('windows')
...
license_groups = data_bag_item.each_with_object('enterprise' => {}) do |(key, value), hash|
  next if %w[id chef_type data_bag].include? key
...
unless node.run_state['cerner_splunk']['total_allotted_pool_size'].nil?
  ...
  raise "Sum of pool sizes is #{CernerSplunk.human_readable_size total_allotted_pool_size}. Exceeds total available pool size of #{CernerSplunk.human_readable_size total_available_license_quota}." if total_allotted_pool_size > total_available_license_quota
end
